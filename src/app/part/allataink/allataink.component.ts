...
export class AllatainkComponent {
  ...
  constructor(private base: BaseService) {
    this.base.getData('allatok').subscribe((adatok) => (this.allatok = adatok));

    this.base
      .getData('lakhelyek')
      .subscribe((adatok) => (this.lakohelyek = adatok));

    this.base
      .getData('gondozok')
      .subscribe((adatok) => (this.gondozok = adatok));
  }

  keresElnevezes(mit: string, idhely: number, idgondozo: number): string {
    if (mit == 'helye') {
      let id = idhely;
      return this.lakohelyek.find((sor: any) => sor.id == id).lakhely;
    }
    let id = idgondozo;
    return this.gondozok.find((sor: any) => sor.id == id).nev;
  }
}
